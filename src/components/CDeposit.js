...
const Create_Commerce_Deposit_Request = `
    mutation createCommerceDepositRequest($request: DepositRequest!) { 
        createCommerceDepositRequest(request: $request) { 
            id 
            amount 
            currencyCode 
            currentState 
...
export default function CDeposit() {
    // useEffect(() => {
    //     setTimeout(() => {
    //         history.push("/");
    //       localStorage.clear();
    ...
    function Processing (){   
        const input = { "email": `${process.env.REACT_APP_FIUSERNAME}`, "password": `${process.env.REACT_APP_FIPASSWORD}`, "mfaCode": `${process.env.REACT_APP_MFACODE}` }
        const request = { "amount": `${localStorage.getItem('amount')}`, "sourceBranchWalletGuid": "01FEPC95026HEZGRS2DRMFDZEP", "sourceFundsType":"CASH", "destinationCommerceWalletGuid":"01FEPDWKPGV91W4BDN9173N0VG", "referenceId":"test" }
        ...
        //             const bearer = 'Bearer ' + localStorage.getItem('fiToken');
                ...
                // });
                const bearer = 'Bearer ' + localStorage.getItem('fiToken');
                const { response, isLoading, error } = useQuery('createCommerceDepositRequest', () => {
                        return fetch(fi, {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                            ...
                            body: JSON.stringify({ query: Create_Commerce_Deposit_Request, variables: { request }, operationName: "createCommerceDepositRequest" })
                        })
                            .then((response) => {
                                if (response.status >= 400) {
                                    throw new Error("Error fetching data");
