...
const login = 'https://api.demo.bittcbdc.com/commerce/login/graphql';
const commerce = 'https://api.demo.bittcbdc.com/commerce/graphql';
...
export default function Landing() {
    ...
    const Transaction = () => {
        return (
            <>
                ...
                <h1>₦{serverResponse.commerceWalletInfo.walletBalance}</h1>
                <p></p>
                <p></p>
                <h4>Wallet Address</h4>
                <h1>{serverResponse.commerceWalletInfo.walletAddress}</h1>
                <div style={{ justifyContent: "center", flexDirection: "column", alignItems: "center", marginTop: 20, }}>
                <Link to={{pathname:'/commercedepo'}}>
                ...
                <div style={{ justifyContent: "center", flexDirection: "column", alignItems: "center", marginTop: 20, }}>
                <Link to={{pathname:'/commercetrans'}}>
                    <button className="btn btn-md btn-info" style={{
                        width: 500,
                        backgroundColor: "#4682B4",
                        padding: 15,
    ...
    const input = { "email": `${creds.email}`, "password": `${creds.password}` }
    // localStorage.setItem('decode', JSON.stringify(jwt_decode(localStorage.getItem('commerceToken'))))
    const { data, isLoading, error } = useQuery('walletHolderLogin', () => {
        return fetch(login, {
            method: "POST",
            ...
            .then((data) => {
                localStorage.setItem('commerceToken', data.data.walletHolderLogin.token)
                localStorage.setItem('decode', JSON.stringify(jwt_decode(localStorage.getItem('commerceToken'))))
                // const userID = localStorage.setItem('decode')
                const bearer = 'Bearer ' + localStorage.getItem('commerceToken');
                return fetch(commerce, {
                    ...
                    body: JSON.stringify({ query: GET_Commerce })
                })
                    .then((response) => {
                        if (response.status >= 400) {
                            throw new Error("Error fetching data");
